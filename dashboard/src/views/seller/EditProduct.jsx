...
const EditProduct = () => {
    ...
    const categorySearch = (e) => {
        ...
        setSearchValue(value);
        if (value) {
            const filteredCategories = categorys.filter(c => c.name.toLowerCase().includes(value));
    ...
    const changeImage = (img, files) => {
        if (files.length > 0) {
            dispatch(product_image_update({ oldImage: img, newImage: files[0], productId }));
        }
    };

    const update = (e) => {
        e.preventDefault();
        dispatch(update_product({ ...state, productId }));
    };
    ...
    return (
        <div className='px-2 lg:px-7 pt-5'>
            <div className='w-full p-4 bg-[#283046] rounded-md'>
                <div className='flex justify-between items-center pb-4'>
                    <h1 className='text-[#d0d2d6] text-xl font-semibold'>Edit Product</h1>
                    <Link className='bg-blue-500 hover:shadow-lg text-white rounded-sm px-7 py-2 my-2' to='/seller/dashboard/products'>Products</Link>
                </div>
                <form onSubmit={update}>
                    {/* Input Fields */}
                    <div className='flex flex-col mb-3 md:flex-row gap-4 w-full text-[#d0d2d6]'>
                        <div className='flex flex-col w-full gap-1'>
                            <label htmlFor="name">Product Name</label>
                            <input className='px-4 py-2 bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.name} type="text" name='name' id='name' placeholder='Product Name' />
                        </div>
                        <div className='flex flex-col w-full gap-1'>
                            <label htmlFor="brand">Product Brand</label>
                            <input className='px-4 py-2 bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.brand} type="text" name='brand' id='brand' placeholder='Product Brand' />
                        </div>
                    </div>
                    ...
                    <div className='flex flex-col mb-3 md:flex-row gap-4 w-full text-[#d0d2d6]'>
                        <div className='flex flex-col w-full gap-1 relative'>
                            <label htmlFor="category">Category</label>
                            <input readOnly onClick={() => setCateShow(!cateShow)} className='px-4 py-2 bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]' value={category} placeholder='--select category--' id='category' />
                            {cateShow && (
                                <div className='absolute top-full bg-slate-800 w-full z-10'>
                                    <input value={searchValue} onChange={categorySearch} className='px-3 py-1 w-full bg-transparent border border-slate-700 rounded-md text-[#d0d2d6]' type="text" placeholder='Search' />
                                    <div className='flex flex-col max-h-60 overflow-auto'>
                                        {allCategory.map((c, i) => (
                                            <span key={i} className={`px-4 py-2 cursor-pointer ${category === c.name ? 'bg-indigo-500' : ''}`} onClick={() => {
                                                setCategory(c.name);
                                                setSearchValue('');
                                                setCateShow(false);
                                            }}>
                        ...
                        <div className='flex flex-col w-full gap-1'>
                            <label htmlFor="stock">Stock</label>
                            <input className='px-4 py-2 bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.stock} type="number" name='stock' id='stock' min='0' placeholder='Product Stock' />
                        </div>
                    </div>
                    ...
                    <div className='flex flex-col mb-3 md:flex-row gap-4 w-full text-[#d0d2d6]'>
                        <div className='flex flex-col w-full gap-1'>
                            <label htmlFor="price">Price</label>
                            <input className='px-4 py-2 bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.price} type="number" name='price' id='price' placeholder='Price' />
                        </div>
                        <div className='flex flex-col w-full gap-1'>
                            <label htmlFor="discount">Discount</label>
                            <input className='px-4 py-2 bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.discount} type="number" name='discount' id='discount' placeholder='% Discount' />
                        </div>
                    </div>
                    ...
                    <div className='flex flex-col w-full gap-1 text-[#d0d2d6] mb-5'>
                        <label htmlFor="description">Description</label>
                        <textarea rows={4} className='px-4 py-2 bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.description} name='description' id='description' placeholder='Description'></textarea>
                    </div>

                    {/* Image Uploads */}
                    <div className='grid lg:grid-cols-4 grid-cols-1 md:grid-cols-3 sm:grid-cols-2 gap-3 w-full text-[#d0d2d6] mb-4'>
                        {imageShow.map((img, i) => (
                            <div key={i}>
                                <label className='h-[180px]'>
                                    <img className='h-full' src={img} alt="" />
                                </label>
                                <input onChange={(e) => changeImage(img, e.target.files)} type="file" className='hidden' />
                            </div>
                        ))}
                    </div>
                    ...
                    {/* Update Button */}
                    <div className='flex'>
                        <button disabled={loader} className='bg-blue-500 w-[190px] hover:shadow-lg text-white rounded-md px-7 py-2 mb-3'>
                            {loader ? <PropagateLoader color='#fff' cssOverride={overrideStyle} /> : 'Update Product'}
                        </button>
                    </div>
                </form>
