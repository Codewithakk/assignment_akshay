...
const CategoryShops = () => {
    ...
    useEffect(() => {
        const fetchPriceRange = async () => {
            try {
                await dispatch(price_range_product());
            } catch (error) {
                                ...
                                <div className='flex flex-col gap-3'>
                                    {[5, 4, 3, 2, 1].map((star) => (
                                        <div key={star} onClick={() => setRatingQ(star)} className='text-orange-500 flex justify-start items-start gap-2 text-xl cursor-pointer'>
                                            {[...Array(star)].map((_, index) => <AiFillStar key={index} />)}
                                            {[...Array(5 - star)].map((_, index) => <CiStar key={index} />)}
                                        </div>
                                    ))}
                                    <div onClick={resetRating} className='text-orange-500 flex justify-start items-start gap-2 text-xl cursor-pointer'>
                                        {[...Array(5)].map((_, index) => <CiStar key={index} />)}
                                    </div>
                                </div>
                            </div>
                            <div className='py-5 flex flex-col gap-4 md:hidden'>
